...
class MIUITask:
    ...
    def post_sign(self, data):
        ...
        s_str = (
            hashlib.md5(str(s_str).encode(encoding="UTF-8")).hexdigest() + "067f0q5wds4"
        )
    ...
    def thumb_up(self):
        ...
        sign_data = {"postId": "36625780", "timestamp": int(round(time.time() * 1000))}
        sign = self.post_sign(sign_data)
        data = {"postId": "36625780", "sign": sign[0], "timestamp": sign[1]}
        try:
            response = requests.get(
                "https://api.vip.miui.com/mtop/planet/vip/content/announceThumbUp",
                headers=headers,
                params=data,
            )
    ...
    def cancel_thumb_up(self):
        ...
        try:
            response = requests.get(
                "https://api.vip.miui.com/mtop/planet/vip/content/announceCancelThumbUp?postId=36625780",
                headers=headers,
            )
            r_json = response.json()
            if r_json["code"] == 401:
    ...
    def browse_post(self):
        ...
        try:
            response = requests.get(
                "https://api.vip.miui.com/mtop/planet/vip/member/addCommunityGrowUpPointByAction",
                params=params,
                headers=headers,
            )
            r_json = response.json()
    ...
    def board_unfollow(self):
        ...
        try:
            response = requests.get(
                "https://api.vip.miui.com/api/community/board/unfollow?"
                "boardId=5462662&pathname=/mio/singleBoard&version=dev.1144",
                headers=headers,
            )
            r_json = response.json()
    ...
    def carrot_pull(self):
        ...
        data = {"miui_vip_ph": str(self.miui_vip_ph)}
        try:
            response = requests.post(
                "https://api.vip.miui.com/api/carrot/pull", headers=headers, data=data
            )
            r_json = response.json()
            if r_json["code"] == 401:
    ...
    def Add_Comment(self):
        headers = {
            "cookie": str(self.cookie),
            "User-Agent": "Mozilla/5.0 (Linux; Android 13; 22081212G Build/TKQ1.220829.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/115.0.5790.166 Mobile Safari/537.36XiaoMi/HybridView/ app/vipaccount/dev.230627",
        ...
        data = {
            ...
            "postId": 40171570,
            "text": str("支持"),
            "extra": '{"product":"","model":"","atUsers":""}',
            "requestId": "",
            "timestamp": int(round(time.time() * 1000)),
            "imagesInfo": "",
        }
        sign = self.post_sign(data)
        data1 = {
            "miui_vip_ph": str(self.miui_vip_ph),
            "postId": 40171570,
            "text": str("支持"),
        ...
        try:
            response = requests.post(
                "https://api.vip.miui.com/mtop/planet/vip/content/addCommentReturnCommentInfo?ref=homepage&pathname=%2Fmio%2Fdetail&version=dev.230627&miui_version=V9.0.1.0.TMKCNXM&android_version=13&oaid=b2ea432544d9c81d&device=diting&restrict_imei=&miui_big_version=V140&model=Redmi%20K50%20Ultra&androidVersion=13&miuiBigVersion=V140&miui_vip_ph="
                + str(self.miui_vip_ph),
                headers=headers,
                data=data1,
            )
            r_json = response.json()
            w_log(r_json)
            if r_json["code"] != 200:
                return w_log("发送评论失败：" + str(r_json["entity"]["message"]))
            w_log("评论结果：" + str(r_json["entity"]["message"]))
        except Exception as e:
            w_log("评论结果：")
            w_log(e)
    ...
    def check_in(self):
        ...
        params = {"miui_vip_ph": str(self.miui_vip_ph)}
        try:
            response = requests.get(
    ...
    def login_app(self):
        ...
        try:
            response = requests.get(
                "https://api.vip.miui.com/mtop/planet/vip/app/init/start/infos",
                headers=headers,
                params=params,
            )
    ...
    def get_point(self) -> int:
        ...
        """
        headers = {"cookie": str(self.cookie)}
        params = {"userId": str(self.uid), "miui_vip_ph": str(self.miui_vip_ph)}
        try:
            response = requests.get(
                "https://api.vip.miui.com/mtop/planet/vip/member/getGrowUpPageData",
                headers=headers,
                params=params,
            )
            r_json = response.json()
            your_point = re.findall(
                r"'title': '成长值'.*'title': '(\d+)'.*'title': '/'", str(r_json["entity"])
            )[0]
...
def start(
    miui_task: MIUITask, check_in: bool, carrot_pull: bool, browse_specialpage: bool
):
    ...
    for i in config.get("accounts"):
        ...
        start(
            MIUITask(
                ...
                i.get("password"),
                i.get("user-agent"),
                device_id=i.get("device-id"),
            ),
            i.get("check-in"),
            i.get("carrot-pull"),
            i.get("browse-specialpage"),
        )
        time.sleep(5)
